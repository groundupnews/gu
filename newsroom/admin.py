...
IMAGE_SIZE_CHOICES = [
    (
        item,
        VERSIONS[item]["verbose_name"],
    )
    for item in ADMIN_VERSIONS
]
IMAGE_SIZE_CHOICES.append(
    (
        "LEAVE",
        "LEAVE",
    )
...
class ArticleForm(forms.ModelForm):
    summary_image_size = forms.ChoiceField(choices=IMAGE_SIZE_CHOICES, initial="medium")
    ...
    def clean(self, *args, **kwargs):
        if self.instance.pk:
            if self.instance.version > self.cleaned_data["version"]:
                raise ValidationError(utils.get_edit_lock_msg(self.instance.user))

        if self.cleaned_data["main_topic"]:
            self.cleaned_data["topics"] = self.cleaned_data[
                "topics"
            ] | models.Topic.objects.filter(name=self.cleaned_data["main_topic"])
...
class ArticleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Content", {"classes": ("wide",), "fields": ("body",)}),
        (
            "Publish",
            {
                "fields": (
    ...
    def new_edit(self, obj):
        return mark_safe(
            '<a href="%s?edit=y">%s</a>' % (obj.get_absolute_url(), obj.title)
        )
    ...
    def old_edit(self, obj):
        return mark_safe(
            '<a href="%s?edit=y">%s</a>'
            % (reverse("admin:newsroom_article_change", args=[obj.pk]), "Old edit")
        )
...
class AuthorAdmin(admin.ModelAdmin):
    ...
    list_editable = ("email", "freelancer", "level", "bonus_off")
    search_fields = [
        "last_name",
        "first_names",
    ]
...
class WetellAdmin(admin.ModelAdmin):
    list_display = ["pk", "service", "published", "created", "modified"]
    ordering = ["-modified"]
    search_fields = ["data"]
