...
DETAIL_TEMPLATE_CHOICES = (("newsroom/article_detail.html", "Standard"),)
...
SECRET_LINK_CHOICES = (
    (
        "n",
        "No prepublication view",
    ),
    (
        "r",
        "Prepublication view only",
    ),
    ("o", "No restrictions"),
)
...
class Author(models.Model):
    ...
    freelancer = models.CharField(max_length=1, default="n", choices=FREELANCER_CHOICES)
    ...
    address = models.TextField(
        blank=True, help_text="Please fill this in. Required by SARS."
    )
    ...
    tax_no = models.CharField(
        max_length=50, blank=True, help_text="Necessary for SARS."
    )
    ...
    vat = models.DecimalField(
        ...
        default=0,
        verbose_name="vat %",
        help_text="If you are VAT regisered set this to 14 else leave at 0",
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk is None or self.user is None:
            ...
            if self.user is None:
                ...
                username = (self.first_names + self.last_name).replace(" ", "_")[0:150]
            ...
            message = render_to_string(
                ...
                {"user": user, "author": self, "pwd": pwd, "site": site},
            )
            try:
                send_mail(
                    subject,
                ...
                log_message = (
                    "Error author creation email failed: "
                    + self.email
                    + "\n"
                    + traceback.print_exc()
                    + "\n"
...
class Topic(models.Model):
    ...
    icon = FileBrowseField("Image", max_length=200, directory="images/", blank=True)
...
class Article(models.Model):
    ...
    summary_image = FileBrowseField(
        "summary image", max_length=200, directory="images/", blank=True
    )
    ...
    audio_summary = FileBrowseField(
        "audio summary", max_length=200, directory="sound/summaries/", blank=True
    )
    audio_publish = models.BooleanField(default=False)
    author_01 = models.ForeignKey(
        Author,
        blank=True,
        null=True,
    ...
    byline_style = models.CharField(max_length=2, choices=BYLINE_CHOICES, default="ST")
    editor_feedback = models.TextField(blank=True)
    primary_image = FileBrowseField(max_length=200, directory="images/", blank=True)
    ...
    external_primary_image = models.CharField(
        blank=True,
    ...
    licence = models.ForeignKey(Licence, null=True, on_delete=models.CASCADE)
    ...
    suppress_ads = models.BooleanField(
        default=False,
    ...
    additional_body_scripts = models.TextField(
        blank=True,
        help_text="Include things like additional javascript "
        "that should come at bottom of article",
    )
    undistracted_layout = models.BooleanField(
        default=False, verbose_name="full width layout"
    )
    ...
    stickiness = models.IntegerField(
        default=0, help_text="The higher the value, the stickier the article."
    )
    slug = models.SlugField(max_length=200, unique=True)

    last_tweeted = models.DateTimeField(
        default=timezone.make_aware(datetime.datetime(year=2000, month=1, day=1))
    )
    # Logging
    created = models.DateTimeField(auto_now_add=True, editable=False)
    modified = models.DateTimeField(auto_now=True, editable=False)
    ...
    author_payment = models.DecimalField(default=0.00, max_digits=9, decimal_places=2)
    ...
    secret_link = models.CharField(
        max_length=64, verbose_name="private URL", blank=True
    )
    secret_link_view = models.CharField(
        choices=SECRET_LINK_CHOICES,
    ...
    cached_byline_no_links = models.CharField(
        max_length=400, blank=True, verbose_name="Byline"
    )
        ...
        if self.published:
            try:
                next_article = (
                    ...
                    .filter(published__gt=self.published)
                    .last()
                )
    ...
    def get_prev_article(self):
        ...
        if self.published:
            try:
                prev_article = (
                    Article.objects.published()
                    .filter(published__lt=self.published)
                    .first()
                )
            except Article.DoesNotExist:
                pass
    ...
    def calc_primary_image(self):
        if self.primary_image:
            if self.primary_image_size == "LEAVE":
                ...
                return self.primary_image.version_generate(self.primary_image_size).url
    ...
    def save(self, *args, **kwargs):
        ...
        self.primary_image_caption = utils.clean_typography(self.primary_image_caption)
        ...
        try:
            if not "sound" in self.audio_summary.name:
                ...
                dest = os.path.join(
                    django_settings.MEDIA_ROOT, django_settings.AUDIO_DIR, fname
                )
...
class UserEdit(models.Model):
    ...
    def __str__(self):
        return ", ".join([str(self.article), str(self.user), str(self.edit_time)])
...
class Correction(models.Model):
    ...
    update_type = models.CharField(
        max_length=1, default="C", choices=CORRECTION_CHOICES
    )
    ...
    notify_republishers = models.BooleanField(
        default=False,
        help_text="Only notifies those that have have already been sent article",
    )
    publishers_notified = models.BooleanField(default=False)
    display_at_top = models.BooleanField(
        default=False,
    ...
    user = models.ForeignKey(
        User, blank=True, null=True, on_delete=models.CASCADE, editable=False
    )
    created = models.DateTimeField(auto_now_add=True, editable=False)
    modified = models.DateTimeField(auto_now=True, editable=False)
...
class WetellBulletin(models.Model):
    ...
    def get_next_bulletin(self):
        ...
        if self.published:
            try:
                next_bulletin = (
                    WetellBulletin.objects.published()
                    .filter(published__gt=self.published)
                    .last()
                )
...
class FlatPageImage(models.Model):
    flatpage = models.OneToOneField(
        FlatPage, on_delete=models.CASCADE, primary_key=True, related_name="image"
    )
    header_image = FileBrowseField(
        "Header Image", max_length=200, directory="images/", blank=True
    )
